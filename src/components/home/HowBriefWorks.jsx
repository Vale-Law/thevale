...
function AttorneyVisual() {
  return (
    <div className="bg-[var(--surface)] rounded-2xl shadow-xl p-5 w-full max-w-xs border border-[var(--line-2)]">
      <div className="flex items-start gap-3 mb-3">
        <div className="w-12 h-12 rounded-full bg-[var(--accent-soft)] flex items-center justify-center shrink-0">
          <span className="font-serif text-[var(--accent)] text-sm font-medium">SM</span>
        </div>
        <div className="flex-1 min-w-0">
          <h4 className="font-serif font-medium text-[14px] text-[var(--text)] truncate">Sarah Mitchell</h4>
          <p className="text-[11px] text-[var(--text-3)] font-body">Family Law</p>
          <div className="flex items-center gap-1 mt-0.5">
            <Star className="w-3 h-3 fill-[#F5A623] text-[#F5A623]" />
            <span className="text-[11px] font-medium text-[var(--text)]">4.9</span>
            <span className="text-[11px] text-[var(--text-3)]">· 127 reviews</span>
          </div>
        </div>
      </div>
      <p className="text-[11px] text-[var(--text-3)] font-body mb-3">Next available today</p>
      <div className="h-8 bg-[var(--surface-sunk)] rounded-full flex items-center justify-center">
        <span className="text-[11px] text-[var(--text)] font-body font-medium">Book online</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-[var(--surface)] rounded-2xl shadow-xl p-5 w-full max-w-xs border border-[var(--line-2)]">
      <div className="flex items-center justify-between mb-3">
        <h4 className="font-serif font-medium text-[14px] text-[var(--text)]">July 2026</h4>
        <Calendar className="w-4 h-4 text-[var(--text-3)]" strokeWidth={1.5} />
      </div>
      <div className="grid grid-cols-7 gap-1 mb-4">
        {dates.map((d, i) => (
          <div key={i} className={`text-center text-[10px] font-body py-1.5 rounded ${i === 2 ? 'bg-[var(--accent)] text-[var(--accent-on)] font-medium' : 'text-[var(--text-3)]'}`}>{d}</div>
        ))}
      </div>
      <div className="space-y-2">
        {slots.map((s, i) => (
          <div key={i} className={`h-7 rounded-lg flex items-center px-3 ${i === 1 ? 'bg-[var(--accent)]' : 'bg-[var(--ground)]'}`}>
            <span className={`text-[11px] font-body ${i === 1 ? 'text-[var(--accent-on)] font-medium' : 'text-[var(--text-3)]'}`}>{s}</span>
          </div>
        ))}
      </div>
    </div>
...
function AccountVisual() {
  return (
    <div className="bg-[var(--surface)] rounded-2xl shadow-xl p-6 w-full max-w-xs border border-[var(--line-2)]">
      <div className="flex items-center gap-3 mb-5">
        <div className="w-12 h-12 rounded-full bg-[var(--accent-soft)] flex items-center justify-center">
          <User className="w-6 h-6 text-[var(--accent)]" strokeWidth={1.5} />
        </div>
        <div className="flex-1">
          <div className="h-2.5 w-20 bg-[var(--surface-sunk)] rounded-full" />
          <div className="h-2 w-14 bg-[var(--surface-sunk)] rounded-full mt-2" />
        </div>
      </div>
      <div className="space-y-2.5">
        <div className="h-9 bg-[var(--ground)] rounded-lg flex items-center px-3">
          <span className="text-[11px] text-[var(--text-3)] font-body">Email address</span>
        </div>
        <div className="h-9 bg-[var(--ground)] rounded-lg flex items-center px-3">
          <span className="text-[11px] text-[var(--text-3)] font-body">Password</span>
        </div>
        <div className="h-9 bg-[var(--accent)] rounded-lg flex items-center justify-center">
          <span className="text-[11px] text-[var(--accent-on)] font-body font-medium">Create account</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-[#1a1d24] rounded-2xl shadow-xl p-5 w-full max-w-xs">
      <div className="aspect-video bg-[#252a33] rounded-lg flex items-center justify-center mb-3 relative">
        <div className="w-14 h-14 rounded-full bg-[var(--accent-soft)] flex items-center justify-center">
          <User className="w-7 h-7 text-[var(--accent)]" strokeWidth={1.5} />
        </div>
      ...
      <div className="flex items-center justify-center gap-2">
        ...
        <div className="h-8 px-4 rounded-full bg-[var(--noshow)] flex items-center justify-center">
...
export default function HowBriefWorks() {
  ...
  /* ----- Pinned scroll-scrub ----- */
  return (
    <section ref={containerRef} className="relative bg-[var(--accent-soft)] h-[400svh]">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-16 w-full">
            ...
            <div className="flex lg:flex-col gap-2 lg:gap-3 justify-center shrink-0">
              {STEPS.map((step, i) => (
                ...
                >
                  <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-[var(--surface)] flex items-center justify-center shrink-0 shadow-sm">
