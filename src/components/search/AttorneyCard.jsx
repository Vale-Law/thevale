...
export default function AttorneyCard({ attorney }) {
  ...
  return (
    <div
      className="bg-[var(--surface)] border border-[var(--line-2)] rounded-xl flex flex-col p-5 w-full h-full min-h-[300px] cursor-pointer"
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <div className="w-[72px] h-[72px] rounded-full overflow-hidden shrink-0 bg-[var(--accent-soft)] flex items-center justify-center">
          {attorney.photo ? (
            <img src={attorney.photo} alt={attorney.name} loading="lazy" className="w-full h-full object-cover" />
          ) : (
            <span className="font-serif text-[var(--accent)] text-lg">{getInitials(attorney.name)}</span>
          )}
        </div>
        <div className="min-w-0 flex-1">
          <h3 className="font-serif font-medium text-[20px] text-[var(--text)] leading-tight truncate">{attorney.name}</h3>
          <p className="text-[13px] text-[var(--text-2)] font-body truncate">{attorney.practice_area}</p>
          <div className="flex items-center gap-1 mt-1 text-[12px]">
            <Star className="w-3 h-3 fill-[#FFC107] text-[#FFC107]" />
            <span className="font-medium text-[var(--text)]">{attorney.rating || '—'}</span>
            <span className="text-[var(--text-2)]">· {attorney.review_count || 0} reviews</span>
          </div>
        </div>
      </div>

      {/* Location line */}
      <div className="flex items-center gap-1 mt-3 text-[12px] text-[var(--text-2)] font-body">
        <MapPin className="w-3 h-3 shrink-0" />
        <span className="truncate">{location}</span>
      </div>

      {/* Tag line */}
      <p className="text-[12px] text-[var(--text-2)] font-body mt-2 line-clamp-2 leading-snug">{tagline}</p>

      {/* Availability line */}
      <p className="text-[13px] font-medium text-[var(--text)] mt-3">
      ...
      {pills.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-3 mb-4">
          {pills.map((p, i) => (
            <span key={i} className="px-3 py-1.5 rounded-full border border-[var(--line-2)] bg-[var(--surface-sunk)] text-[13px] text-[var(--text)] font-body">
              {p}
            </span>
          ))}
        </div>
      ...
      <button
        ...
        className="mt-auto w-full h-12 rounded-lg bg-[var(--accent)] hover:bg-[var(--accent-press)] text-[var(--accent-on)] text-[13px] font-medium font-body transition-colors"
      >
        Book online
      </button>
    </div>
