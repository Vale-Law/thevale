...
export default function QuestionnaireModal({ open, areaLabel, location, onClose, onComplete }) {
  ...
  const LoadingView = (
    <div className="flex flex-col items-center justify-center py-16 md:py-20">
      ...
      <p className="font-serif text-xl text-[var(--text)]">{t('questionnaire.findingMatches')}</p>
      <p className="text-sm text-[var(--text-3)] font-body mt-1">{t('questionnaire.sorting')}</p>
    </div>
  );

  const LangPrefView = (
    <>
      <p className="text-[11px] uppercase tracking-[0.12em] text-[var(--accent)] font-body mb-2">{t('langPref.title')}</p>
      <h2 className="font-serif text-[24px] lg:text-[28px] text-[var(--text)] leading-[1.15] mb-4">
        {t('langPref.question')}
      </h2>
      <div className="flex flex-col gap-2.5 mb-4">
        {[t('langPref.yes'), t('langPref.no')].map((option) => {
          ...
          return (
            ...
            >
              <span>{option}</span>
              <ArrowRight className="w-4 h-4" style={{ color: selected ? 'var(--accent)' : 'var(--text-3)' }} />
            </button>
          );
        })}
      </div>
      <p className="text-sm text-[var(--text-3)] font-body leading-relaxed">{t('langPref.explanation')}</p>
    </>
  );
  ...
  const QuestionView = (
    <>
      {step === 0 && (
        <p className="text-sm text-[var(--text-3)] font-body mb-4 leading-relaxed">{t('questionnaire.intro')}</p>
      )}
      <h2 className="font-serif text-[26px] lg:text-[28px] text-[var(--text)] leading-[1.15] mb-6">
  ...
  if (!isMobile) {
    return (
      <>
        ...
        <div
          className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[2300] bg-[var(--ground)] w-[92vw] max-w-[560px] flex flex-col"
          style={{ borderRadius: 12, boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}
        >
          <div className="flex items-start justify-between px-6 pt-6 pb-2">
            <div>
              <p className="text-[11px] uppercase tracking-[0.12em] text-[var(--accent)] font-body mb-0.5">{areaLabel}</p>
              {location?.city && (
                <p className="text-[11px] text-[var(--text-3)] font-body">{location.city}, {location.state}</p>
              )}
            </div>
            <div className="flex items-center gap-4">
              <button onClick={handleSkip} className="text-sm text-[var(--accent)] hover:text-[var(--text)] transition-colors font-body whitespace-nowrap">
                {t('questionnaire.skipToResults')}
              </button>
              <button onClick={onClose} className="text-[var(--text-3)] hover:text-[var(--text)] transition-colors" aria-label="Close">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
          ...
          {!loading && (
            <div className="px-6 pb-2">
              <div className="flex items-center gap-3">
                <div className="flex-1 h-1 bg-[var(--surface-sunk)] overflow-hidden">
                  <div className="h-full bg-[var(--accent)] transition-all duration-300" style={{ width: `${progress}%` }} />
                </div>
                <span className="text-[11px] text-[var(--text-3)] font-body whitespace-nowrap">
          ...
          {!loading && (
            <div className="px-6 py-4 border-t border-[var(--line-2)]">
              <p className="text-[11px] text-[var(--text-3)] font-body text-center leading-relaxed">{t('questionnaire.footer')}</p>
            </div>
          )}
        </div>
      </>
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 z-[2300] bg-[var(--ground)] flex flex-col"
      ...
      >
        <div className="flex items-center justify-between px-3 h-12 shrink-0">
          ...
          <p className="text-[11px] uppercase tracking-[0.12em] text-[var(--accent)] font-body">{areaLabel}</p>
          <button onClick={handleSkip} className="text-sm text-[var(--accent)] font-body px-2 whitespace-nowrap">
            {t('questionnaire.skipToResults')}
          </button>
        </div>
        ...
        {!loading && (
          <div className="px-5 pb-3 shrink-0">
            <div className="h-1 bg-[var(--surface-sunk)] rounded-full overflow-hidden">
              <div className="h-full bg-[var(--accent)] transition-all duration-300" style={{ width: `${progress}%` }} />
            </div>
          </div>
        )}
